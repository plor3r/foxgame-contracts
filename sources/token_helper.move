module fox_game::token_helper {
    ...
    fun init_traits(trait_data: &mut Table<u8, Table<u8, Trait>>, ctx: &mut TxContext) {
        ...
        upload_traits(trait_data , 10, vector<u8>[0], vector[
    ...
    fun token_uri(reg: &mut FoCRegistry, foc: &Traits): vector<u8> {
        // let metadata = b"";
        // vec::append(&mut metadata, b"{\"name\": \"");
        // vec::append(&mut metadata, if (foc.is_chicken) b"Chicken #" else b"Fox #");
        // vec::append(&mut metadata, to_vector(reg.foc_born));
        // vec::append(&mut metadata, b"\", \"description\": \"Thousands of Chicken and Foxes compete on a farm in the metaverse. A tempting prize of $EGG awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the SUI blockchain.\", \"image\": \"data:image/svg+xml;base64,");
        // vec::append(&mut metadata, base64::encode(&draw_svg(reg, foc)));
        // vec::append(&mut metadata, b"\", \"attributes\":");
        // vec::append(&mut metadata, compile_attributes(reg, foc));
        // vec::append(&mut metadata, b"}");
        // let uri = b"data:application/json;base64,";
        // vec::append(&mut uri, base64::encode(&metadata));
    ...
    fun draw_svg(reg: &mut FoCRegistry, fc: &Traits): vector<u8> {
        ...
        let svg = b"";
        vec::append(&mut svg, b"<svg id=\"woolf\" width=\"100%\" height=\"100%\" version=\"1.1\" viewBox=\"0 0 40 40\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">");
    ...
    fun compile_attributes(reg: &mut FoCRegistry, fc: &Traits): vector<u8> {

        let s = vector[fc.fur, fc.head, fc.ears, fc.eyes, fc.nose, fc.mouth, fc.neck, fc.feet, fc.alpha_index];
        let traits = b"";
        if (fc.is_chicken) {
            let types = vector[0, 1, 2, 3, 4, 5, 7];
            let i = 0;
            while (i < vec::length(&types)) {
                let index = *vec::borrow(&types, i);
                vec::append(&mut traits, attribute_for_type_and_value(
                    *vec::borrow(&reg.trait_types, index),
                    table::borrow(
                        table::borrow(&reg.trait_data, (index as u8)),
                        *vec::borrow(&s, index)
                    ).name
                ));
                vec::append(&mut traits, b",");
                i = i + 1;
            };
        } else {
            ...
            while (i < vec::length(&types)) {
                ...
                vec::append(&mut traits, attribute_for_type_and_value(
                    *vec::borrow(&reg.trait_types, index),
                    table::borrow(
                        table::borrow(&reg.trait_data, (index as u8) + 9),
                        *vec::borrow(&s, *vec::borrow(&sindice, i))
                    ).name
                ));
                vec::append(&mut traits, b",");
                i = i + 1;
            ...
            vec::append(&mut traits, attribute_for_type_and_value(
                b"Alpha",
                // *vec::borrow(&ALPHAS, (*vec::borrow(&s, 8) as u64)) // alpha_index
                    to_vector((*vec::borrow(&ALPHAS, (*vec::borrow(&s, 8) as u64)) as u64))
            ));
            vec::append(&mut traits, b",");
        };
        let attributes = b"";
        vec::append(&mut attributes, b"[");
        vec::append(&mut attributes, traits);
        vec::append(&mut attributes, b"{\"trait_type\":\"Generation\",\"value\":");
        vec::append(&mut attributes, if (fc.is_chicken) b"\"Gen 0\"" else b"\"Gen 1\"");
        vec::append(&mut attributes, b"},{\"trait_type\":\"Type\",\"value\":");
        vec::append(&mut attributes, if (fc.is_chicken) b"\"Chicken\"" else b"\"Fox\"");
        vec::append(&mut attributes, b"}]");

        attributes
    }

    fun attribute_for_type_and_value(trait_type: vector<u8>, value: vector<u8>): vector<u8> {
        let s = b"";
        vec::append(&mut s, b"{\"trait_type\":\"");
        vec::append(&mut s, trait_type);
        vec::append(&mut s, b"\",\"value\":\"");
        vec::append(&mut s, value);
        vec::append(&mut s, b"\"}");
        s
    }
}
